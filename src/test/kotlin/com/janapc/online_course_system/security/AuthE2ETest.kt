...
class AuthE2ETest {
	...
	inner class RegisterTests {
		...
		fun shouldReturn409WhenEmailAlreadyExists() {
			...
			mockMvc
				.perform(post("/auth/register").json(request))
				.andExpect(status().isCreated)
		...
		fun shouldReturn400WhenRegisterRequestIsInvalid() {
			...
			mockMvc
				.perform(post("/auth/register").json(invalidRequest))
	...
	inner class LoginTests {
		...
		fun shouldLoginSuccessfully() {
			...
			mockMvc
				.perform(post("/auth/register").json(registerRequest))
				.andExpect(status().isCreated)

			val loginRequest = LoginRequest(email = "login_success@test.com", password = "password123")

			mockMvc
				.perform(post("/auth/login").json(loginRequest))
		...
		fun shouldReturn401WhenPasswordIsIncorrect() {
			...
			mockMvc
				.perform(post("/auth/login").json(invalidLoginRequest))
